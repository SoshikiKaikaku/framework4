...
include dirname(__FILE__) . '/../../classes/app/common/FFM.php';
...
class fixed_file_manager implements FFM {
	...
	function select($itemname,$value,$exact_match=true,$and_or="AND",$sortitem=null,$sort_order=SORT_DESC,$max=null,&$is_last=null){
		...
		if(count($itemname) == 0){
			throw new Exception("itemname is needed");
		}
		...
		foreach($this->format as $f){
			...
			if($f["name"] != "id"){
				...
				if($f["type"]=="N"){
					...
					$arr[$f["name"]] = json_decode($arr[$f["name"]]);
				}
			}
		}
	}
		...
		foreach($arr as $ar){
			foreach($this->prohibition_item_name as $c){
				...
				if(!preg_match("/^[a-z0-9_]+$/", $ar["name"])){
					throw new Exception("Format Error : This item has prohibited character " . $c . " in " . $this->filename . ".fmt");
				}
			}
		}
